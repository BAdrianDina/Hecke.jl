...
function class_group_proof(clg::ClassGrpCtx, lb::ZZRingElem, ub::ZZRingElem; extra::ZZRingElem=ZZRingElem(0), prec::Int = 100, do_it=1:ub, try_parallel = true)
  ...
  if nt == 1 || try_parallel == false
    ...
    @sync for i in 1:length(part)
        FB = deepcopy(clg.FB)
        _part = part[i]
        Threads.@spawn :default class_group_proof_raw(clg, ZZ(first(_part)), ZZ(last(_part)); extra, prec, filter = Returns(true), clgFB = FB, used_threads = length(part), do_it = first(_part):last(_part))
    end
  end
end

function class_group_proof_raw(clg::ClassGrpCtx, lb::ZZRingElem, ub::ZZRingElem; extra::ZZRingElem=ZZRingElem(0), prec::Int = 100, do_it=1:ub, filter = Returns(true), clgFB = clg.FB, used_threads = 1)
